...
# Dumps all lexical variable from the stack frame with the given offset.
#
# @param frame_index    0 = top stack frame (in user's code),
#                       1 = next lower frame etc.
#
...
sub _dump_keys
{
    ...
    {
        my $tmp = $h->{$key};
        my $val = $add_ref ? \$tmp : $tmp;

        _dump_entity($key, $val);
    }
}
