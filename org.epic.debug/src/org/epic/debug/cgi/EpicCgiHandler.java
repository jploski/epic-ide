...
public class EpicCgiHandler implements Handler {
	...
	//jjjjjj
	command[0] = request.props.getProperty(propsPrefix + EXECUTABLE, "perl");
	command[1] = "-d"; // <- This should be -d
	command[2] = name.getAbsolutePath();
	request.log(Server.LOG_DIAGNOSTIC,propsPrefix + " command= " + command[0]);
	...
	env.addElement("REMOTE_ADDR=" + request.getSocket().getInetAddress().getHostAddress());
	...
	request.log(Server.LOG_DIAGNOSTIC,propsPrefix + " ENV= " + env);
     ...
     //jjjjjj
     // env.addElement("REMOTE_XXX=" + request.props.getProperty("remoteport" + EXECUTABLE, "4444"));
