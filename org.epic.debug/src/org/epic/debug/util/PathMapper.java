...
public class PathMapper
{
    ...
    public IPath mapPath(IPath fPath)
    {
        ...
        if (erg != null) return new Path(erg);
    ...
    private String mapPath(String fOrg)
    {
        ...
        {
            PathMapping map = ((PathMapping) mMapping.get(i));
            if (fOrg.indexOf(map.getOriginal()) == 0)
            {
                erg = map.getMapped()
                    + fOrg.substring(map.getOriginal().length());
                return (erg);
            }
            erg = mapInc(fOrg);
